...
import torch.optim
import os
from nvd_utils import NvdFourierFeatureTransform
from nvd_utils import device


class NvdProgressiveEncoding(nn.Module):
    def __init__(self, mapping_size, T, d=3, apply=True):
        super(NvdProgressiveEncoding, self).__init__()
        self._t = 0
        self.n = mapping_size
        self.T = T
        self.d = d
...
class NvdNeuralStyleField(nn.Module):
    ...
    def __init__(self, sigma, depth, width, encoding, colordepth=2, normdepth=2, normratio=0.1, clamp=None,
        ...
        super(NvdNeuralStyleField, self).__init__()
        self.pe = NvdProgressiveEncoding(mapping_size=width, T=niter, d=input_dim)
        self.clamp = clamp
        self.normclamp = normclamp
        self.normratio = normratio
        layers = []
        if encoding == 'gaussian':
            layers.append(NvdFourierFeatureTransform(input_dim, width, sigma, exclude))
